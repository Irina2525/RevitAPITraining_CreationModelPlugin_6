...
namespace RevitAPITraining_CreationModelPlugin
{
    ...
    public class Main : IExternalCommand   
    {
        ...
        private static void AddRoof(Document doc, Level level2, List<Wall> walls) // создаем крышу
        {
            ...
            //            double dw = wallWidth / 2; // делим пополам что бы получить смещение
            //            List<XYZ> points = new List<XYZ>(); // массив точек
            ...
            //                XYZ p1 = curve.Curve.GetEndPoint(0); // получаем две точки p1 p2 
            //                XYZ p2 = curve.Curve.GetEndPoint(1);
            //                Line line = Line.CreateBound(p1 + points[i], p2 + points[i + 1]); // получаем новую линию
